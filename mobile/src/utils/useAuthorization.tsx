...
const CHAIN_IDENTIFIER = `solana:${CLUSTER}` as Chain;
...
export function useAuthorization() {
  ...
  const authorizeSessionWithSignIn = useCallback(
    async (wallet: AuthorizeAPI, signInPayload: SignInPayload) => {
      ...
      const walletAuth = await handleAuthorizationResult(authorizationResult);
      // Attach sign_in_result to the account so callers can access the signature
      const signInResult = (authorizationResult as any).sign_in_result;
      if (signInResult) {
        return {
          ...walletAuth.selectedAccount,
          signInResult: {
            address: signInResult.address,
            signed_message: signInResult.signed_message,
            signature: signInResult.signature,
          },
        } as Account;
      }
      return walletAuth.selectedAccount;
    },
    [authorization, handleAuthorizationResult]
  );
